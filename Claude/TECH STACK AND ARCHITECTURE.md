...
4. **Review-screen auto-fill and candidate narrowing** — the review screen never presents a bare barcode; several fields start pre-filled as editable guesses, and the OMDB candidate list itself is narrowed before the user ever sees it:
   ...
   - Explicitly **not implemented**, and not a near-term plan: automatically matching the listing's own photo against OMDB posters (e.g. via perceptual-image-hashing) to auto-pick the correct candidate, and listing alternate physical releases/editions of a film once identified. The second one hits a hard data-availability wall rather than an effort one — OMDB holds exactly one entry per film with no per-edition/per-pressing data at all, and no API in this stack tracks that (Blu-ray.com, which would have it, has no API and is already excluded entirely — see above). The first is technically feasible (a real photo vs. a stylized theatrical poster can plausibly be compared) but was deliberately not built without discussing the real accuracy tradeoff first, since a resale-listing photo often looks quite different from OMDB's poster even for the objectively correct film.
