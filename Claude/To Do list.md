...
TMDb attribution: once the actual browsable web app (Phase 2+) displays Rating/Studio data pulled from TMDb (see Claude/TECH STACK AND ARCHITECTURE.md's TMDb section), TMDb's API Terms of Use require showing their logo plus the exact notice "This [product] uses TMDB and the TMDB APIs but is not endorsed, certified, or otherwise approved by TMDB" (less prominent than the app's own branding). Not needed for the mobile scan-confirm tool itself (private, single-owner), only for the public-facing web UI once it exists.

Collection-wide misspelling audit: the user asked for a read-only pass over the whole collection looking for misspelled titles, using the fuzzy TMDb title indexes (tmdb_title_index for movies, tmdb_tv_title_index for TV/documentary - both new, see TECH STACK AND ARCHITECTURE.md). Found well over 100 confirmed one-off typos plus several systematic ones affecting many rows at once (e.g. "Carribbean"->"Caribbean" across the Pirates of the Caribbean set, "Abbot"->"Abbott" across ~16 Abbott & Costello entries, "Tripple"->"Triple" across 11 box-set descriptions, "Downtown Abbey"->"Downton Abbey"). Full curated list was reported in chat rather than written here - nothing has been changed in the Sheet/database yet; the user said they'd decide what to do with the list once they'd seen it. If picked back up later: still needs a final pass reconciling the list against the real Sheet (to write actual corrections) and a decision on whether to fix the ~100+ lower-confidence/ambiguous cases too.
